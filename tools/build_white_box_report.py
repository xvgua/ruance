...
def main():
    ...
    replace_paragraph(
        refs["summary_hint"],
        "本人：通过本次实验，我主要负责血压评估和血糖评估模块的测试工作，对白盒测试和黑盒测试的区别与联系有了更清晰的认识。在白盒测试部分，我根据代码中的判断条件设计了语句覆盖、分支覆盖、条件覆盖、路径覆盖和边界值测试用例，重点验证了血压 120/80、140/90 以及血糖 3.9、6.1、7.0、7.8、11.1 等关键边界。通过 JUnit 自动化测试，我体会到单元测试能够快速、稳定地验证业务逻辑，也能帮助发现边界条件和异常处理是否充分。在黑盒测试部分，我从用户界面输入和输出的角度设计测试用例，重点检查正常输入、非法输入、错误提示和不同时段血糖判断结果，进一步认识到功能正确不仅体现在代码逻辑中，也体现在用户操作流程和提示信息是否清晰。",
    )
    replace_paragraph(
        refs["summary_a"],
        "本次实验的收获是，我学会了将需求中的规则转化为可执行的测试用例，并能够结合等价类划分、边界值分析、错误推测法等方法提高测试的完整性。实验过程中也发现，健康类数据的边界判断非常容易出现遗漏，如果只测试普通数据，很难证明程序在临界输入下也是可靠的。后续如果继续完善该模块，我会进一步补充界面自动化测试和测试截图记录，并对提示文字、输入限制和用户操作体验进行更细致的验证，使测试过程更加规范、结果更加有说服力。",
    )
    clear_paragraph(refs["summary_b"])
    clear_paragraph(refs["summary_note"])
